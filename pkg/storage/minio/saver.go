...
func (s *storageImpl) Save(ctx context.Context, module, vsn string, mod []byte, zip io.Reader, zipMD5, info []byte) error {
	const op apierrors.Op = "storage.minio.Save"
...
func (zw *partWriter) Write(p []byte) (int, error) {
	const op apierrors.Op = "minio.partWriter.Write"
	if zw.err != nil {
		return 0, apierrors.E(op, zw.err)
	} else if len(p) == 0 {
		return 0, nil
	}
...
func (s *storageImpl) saveZip(dir, mod, ver string, zip io.Reader) error {
	const op apierrors.Op = "minio.saveZip"
	...
	if err != nil {
		return apierrors.E(op, apierrors.E("minio.NewDestinationInfo", err))
	}
	err = s.minioClient.ComposeObject(dst, wr.srcs)
	if err != nil {
		return apierrors.E(op, apierrors.E("minio.ComposeObject", err))
	}
...
func (s *storageImpl) removeParts(mod, ver string, numParts int) error {
	const op apierrors.Op = "minio.removeParts"
