...
func (s *ModuleStore) Info(ctx context.Context, module, vsn string) ([]byte, error) {
	const op apierrors.Op = "mongo.Info"
...
func (s *ModuleStore) GoMod(ctx context.Context, module, vsn string) ([]byte, error) {
	const op apierrors.Op = "mongo.GoMod"
	ctx, span := observ.StartSpan(ctx, op.String())
	defer span.End()
...
func (s *ModuleStore) Zip(ctx context.Context, module, vsn string) (storage.SizeReadCloser, error) {
	const op apierrors.Op = "mongo.Zip"
	ctx, span := observ.StartSpan(ctx, op.String())
	defer span.End()
	...
	if err != nil {
		...
		return nil, apierrors.E(op, err, kind, apierrors.M(module), apierrors.V(vsn))
	}
	res := s.client.Database(s.db).Collection("fs.files").FindOne(ctx, bson.M{
		"filename": zipName,
	})
	if res.Err() != nil {
		return nil, apierrors.E(op, res.Err())
	}
	var m bson.M
	err = res.Decode(&m)
	if err != nil {
...
func query(ctx context.Context, s *ModuleStore, module, vsn string) (*storage.Module, error) {
	const op apierrors.Op = "mongo.query"
	ctx, span := observ.StartSpan(ctx, op.String())
	defer span.End()
	...
	if queryErr := queryResult.Err(); queryErr != nil {
		...
		return nil, apierrors.E(op, queryErr, kind, apierrors.M(module), apierrors.V(vsn))
	}
