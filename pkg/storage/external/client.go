...
func (s *service) List(ctx context.Context, mod string) ([]string, error) {
	const op apierrors.Op = "external.List"
	...
	if scnr.Err() != nil {
		return nil, apierrors.E(op, scnr.Err())
	}
	return list, nil
}

func (s *service) Info(ctx context.Context, mod, ver string) ([]byte, error) {
	const op apierrors.Op = "external.Info"
...
func (s *service) GoMod(ctx context.Context, mod, ver string) ([]byte, error) {
	const op apierrors.Op = "external.GoMod"
...
func (s *service) Zip(ctx context.Context, mod, ver string) (storage.SizeReadCloser, error) {
	const op apierrors.Op = "external.Zip"
...
func (s *service) Save(ctx context.Context, mod, ver string, modFile []byte, zip io.Reader, zipMD5, info []byte) error {
	const op apierrors.Op = "external.Save"
	var err error
	...
	if resp.StatusCode != http.StatusOK {
		...
		return apierrors.E(op, fmt.Errorf("unexpected status code: %v - body: %s", resp.StatusCode, bts), resp.StatusCode)
	}
	return nil
}

func (s *service) Delete(ctx context.Context, mod, ver string) error {
	const op apierrors.Op = "external.Delete"
...
func (s *service) doRequest(ctx context.Context, method, mod, ver, ext string) (io.ReadCloser, int64, error) {
	const op apierrors.Op = "external.doRequest"
	var err error
	mod, err = module.EscapePath(mod)
	if err != nil {
		return nil, 0, apierrors.E(op, err)
	}
	url := s.url + "/" + mod + "/@v/" + ver
	if ext != "" {
		url += "." + ext
	...
	if resp.StatusCode != http.StatusOK {
		...
		_ = resp.Body.Close()
		return nil, 0, apierrors.E(op, fmt.Errorf("none 200 status code: %v - body: %s", resp.StatusCode, body), resp.StatusCode)
	}
	var size int64
	if cl := resp.Header.Get("Content-Length"); cl != "" {
		size, err = strconv.ParseInt(cl, 10, 64)
		if err != nil {
			return nil, 0, apierrors.E(op, fmt.Errorf("could not parse content-length(%q): %w", cl, err))
		}
	}
